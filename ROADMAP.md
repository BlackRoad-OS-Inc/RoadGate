...
- Deploy RoadGate to Alice (.49) with systemd auto-start
...
- Add NLP intent routing: 'route model' / 'llm gateway'
- Create RoadGate Agent persona with RoundTrip auto-chat
- Integrate with RoadMem for persistent state across restarts
- Add Codex solution entries for common RoadGate operations
- Build blackroad-operator CLI commands: `br RoadGate status|deploy|logs`
...
- Contribute BlackRoad improvements back to upstream TensorZero
